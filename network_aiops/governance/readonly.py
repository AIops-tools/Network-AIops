...
Set ``NETWORK_READ_ONLY=1`` (or ``true``/``yes``/``on``) and this tool can only
read. Enforcement is deliberately two-layered:

  1. :func:`~network_aiops.governance.decorators.governed_tool` refuses any
...
READ_ONLY_ENV = "NETWORK_READ_ONLY"
