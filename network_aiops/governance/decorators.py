...
Responsibilities:
  ...
  3. Post-log: write audit record to ``~/.network-aiops/audit.db``
...
from network_aiops.governance.audit import detect_agent, get_engine
from network_aiops.governance.budget import BudgetExceeded, get_budget
from network_aiops.governance.outcome import (
    clear_prior_state,
    is_unknown,
    take_prior_state,
)
from network_aiops.governance.patterns import PatternMatch, get_pattern_engine
from network_aiops.governance.policy import PolicyResult, get_policy_engine
from network_aiops.governance.readonly import is_read_only, read_only_denial
from network_aiops.governance.sanitize import sanitize

_log = logging.getLogger("network-aiops.decorators")
    ...
    *,
    ...
    Args:
        ...
        undo: Optional callable ``(params, result) -> dict | None`` returning an
            ...
            successful call the inverse is recorded to ~/.network-aiops/undo.db and the
...
class _CallState:
    ...
    def __init__(
        ...
        self.rationale = os.environ.get("NETWORK_AUDIT_RATIONALE", "")
        self.approved_by = os.environ.get("NETWORK_AUDIT_APPROVED_BY", "")
        self.risk_tier = ""
...
def _pre_check(state: _CallState) -> None:
    ...
    # review) are denied when none was recorded (NETWORK_AUDIT_APPROVED_BY).
    ...
    if tier.requires_approver and not state.approved_by and not state.dry_run:
        reason = (
            ...
            f"Set NETWORK_AUDIT_APPROVED_BY to the authorizing human (and "
            f"NETWORK_AUDIT_RATIONALE to why) before retrying."
        )
        if tier.reason:
...
def _record_undo(state: _CallState, result: Any) -> None:
    ...
    * **Undetermined** — the response was lost (see
      :mod:`network_aiops.governance.outcome`). The change may be live, so an
    ...
    if not descriptor:
        return
    try:
        from network_aiops.governance.undo import get_undo_store
...
def _infer_skill(func: Any) -> str:
    ...
    ``network_aiops.ops.jobs`` → ``network-aiops``
    ``mcp_server.server`` → ``network-aiops`` (the only consumer here).
    """
    module = getattr(func, "__module__", "") or ""
    if module.startswith("network_aiops") or module.startswith("mcp_server"):
        return "network-aiops"
    return "unknown"
