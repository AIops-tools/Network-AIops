...
Rules are loaded from ``~/.network-aiops/rules.yaml`` with hot-reload on file change.
"""
...
_log = logging.getLogger("network-aiops.policy")
...
class PolicyEngine:
    ...
    def check_allowed(
        ...
        *,
        ...
        if os.environ.get("NETWORK_POLICY_DISABLED") == "1":
        ...
        if not tiers:
            if not self._rules and risk_level in ("high", "critical"):
                return TierDecision(
                    ...
                    reason=(
                        f"No rules file at {self._path} — high/critical operations require a named approver (NETWORK_AUDIT_APPROVED_BY) by default. Author rules.yaml "
