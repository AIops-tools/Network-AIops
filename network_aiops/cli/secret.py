"""``network-aiops secret`` — manage the encrypted credential store.

Secrets are stored in ``~/.network-aiops/secrets.enc`` (Fernet, key derived from
a master password via scrypt). Device login passwords are keyed by the device
name; the NetBox API token uses the reserved name ``netbox-token``. Nothing here
...
from network_aiops.config import SECRET_ENV_PREFIX, SECRET_ENV_SUFFIX
...
NameArg = Annotated[
    str,
    typer.Argument(
        help="Device name the password belongs to, or 'netbox-token' for the NetBox API token"
    ),
]
...
def secret_set(
    ...
    """Store (or replace) a device password or the NetBox token — value read hidden."""
...
def secret_list() -> None:
    """List names that have a stored secret (values never shown)."""
    store = SecretStore.unlock()
    names = store.names()
    if not names:
        console.print(
            "[yellow]No secrets stored yet. Add one: network-aiops secret set <name>[/]"
        )
        return
...
def secret_migrate() -> None:
    ...
    Imports per-device ``NETWORK_<NAME>_PASSWORD`` entries and the single
    ``NETWORK_NETBOX_TOKEN`` (stored as 'netbox-token').
    """
    password = resolve_master_password(confirm_if_new=True)
...
def secret_rotate_password() -> None:
    ...
    console.print("[green]✓ Master password rotated. Update NETWORK_AIOPS_MASTER_PASSWORD.[/]")
