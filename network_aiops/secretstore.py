"""Encrypted secret storage for network-aiops.

Secrets (per-device login passwords and the NetBox API token) are NEVER written
to disk in plaintext. They live in ``~/.network-aiops/secrets.enc`` encrypted
with Fernet (AES-128-CBC + HMAC-SHA256). The Fernet key is derived from a master
password via scrypt — the password itself is never stored; only a random
per-store salt and the ciphertext are on disk.

Two kinds of secret share the one store:

  * per-device login passwords, keyed by the device target name (e.g.
    ``core-sw1``), and
  * a single NetBox API token, keyed by the reserved name ``netbox-token``.

The master password is resolved (in order) from:

  1. the env var ``NETWORK_AIOPS_MASTER_PASSWORD`` (for non-interactive use:
...
APP_NAME = "network-aiops"
CONFIG_DIR = ops_home()
MASTER_PASSWORD_ENV = "NETWORK_AIOPS_MASTER_PASSWORD"  # nosec B105 — env var name, not a secret
# ──────────────────────────────────────────────────────────────────────────────

# The reserved secret name for the single NetBox API token.
NETBOX_TOKEN_NAME = "netbox-token"  # nosec B105 — store key name, not a secret
...
def migrate_legacy_env(
    ...
    *,
    netbox_env: str | None = "NETWORK_NETBOX_TOKEN",
    netbox_secret_name: str = NETBOX_TOKEN_NAME,
) -> list[str]:
    """Import plaintext ``.env`` secrets into the encrypted store.

    Per-device legacy keys look like ``NETWORK_<TARGET>_PASSWORD``;
    ``prefix='NETWORK_'`` and ``suffix='_PASSWORD'`` map them back to a target
    name. The single NetBox token key (``netbox_env``) is imported under the
    reserved ``netbox_secret_name``. The plaintext ``.env`` is renamed to
    ``.env.migrated`` (chmod 600) so nothing is silently lost. Returns the list
    of imported secret names.
    """
    ...
    for line in LEGACY_ENV_FILE.read_text("utf-8").splitlines():
        ...
        if not value:
            continue
        if netbox_env and key == netbox_env:
            store = store.set(netbox_secret_name, value)
            imported.append(netbox_secret_name)
            continue
