...
``network_aiops.governance.*`` and binds state to an isolated home via
``NETWORK_AIOPS_HOME``. Every governed function under test is SYNTHETIC and
...
import network_aiops.governance.budget as budget_mod
import network_aiops.governance.decorators as dec_mod
...
from network_aiops.governance.budget import BudgetExceeded
...
def dec_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("NETWORK_MAX_TOOL_CALLS", raising=False)
    monkeypatch.delenv("NETWORK_MAX_TOOL_SECONDS", raising=False)
    _reset()
    yield tmp_path
    _reset()
...
def test_budget_exceeded_propagates_and_audits(dec_home, monkeypatch):
    monkeypatch.setenv("NETWORK_MAX_TOOL_CALLS", "0")
...
def test_policy_disabled_marks_status_bypassed(dec_home, monkeypatch):
    monkeypatch.setenv("NETWORK_POLICY_DISABLED", "1")
    _reset()
