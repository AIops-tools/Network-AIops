...
nothing touches the real ``~/.network-aiops``.
"""
...
def test_roundtrip_set_get(store_dir):
    ...
    store = store.set("core-sw1", "device-pw-123")
    reopened = ss.SecretStore.unlock("master-pw")
    assert reopened.get("core-sw1") == "device-pw-123"


@pytest.mark.unit
def test_value_not_plaintext_on_disk(store_dir):
    ss.SecretStore.unlock("master-pw").set("core-sw1", "super-secret-value")
...
@pytest.mark.unit
def test_netbox_token_not_plaintext_on_disk(store_dir):
    ss.SecretStore.unlock("pw").set(ss.NETBOX_TOKEN_NAME, "netbox-api-token-xyz")
    blob = (store_dir / "secrets.enc").read_text()
    assert "netbox-api-token-xyz" not in blob
    assert ss.SecretStore.unlock("pw").get(ss.NETBOX_TOKEN_NAME) == "netbox-api-token-xyz"
...
@pytest.mark.unit
def test_migrate_legacy_env_devices_and_netbox(store_dir):
    (store_dir / ".env").write_text(
        "NETWORK_CORE_SW1_PASSWORD=legacy-pw\n"
        "NETWORK_NETBOX_TOKEN=legacy-token\n"
        "# comment\nFOO=bar\n"
    )
    imported = ss.migrate_legacy_env("NETWORK_", "_PASSWORD", "pw")
    assert "core-sw1" in imported
    assert ss.NETBOX_TOKEN_NAME in imported
    reopened = ss.SecretStore.unlock("pw")
    assert reopened.get("core-sw1") == "legacy-pw"
    assert reopened.get(ss.NETBOX_TOKEN_NAME) == "legacy-token"
