...
a high-risk write runs and is audited whether or not an approver annotation is set.
"""
...
from network_aiops.governance import (
    capture_prior_state,
    governed_tool,
    mark_unknown,
)
...
def test_high_risk_runs_without_an_approver_and_is_audited(gov_home):
    ...
    result = _drop_widget(name="w2", target="core-sw1")
...
def test_real_write_tool_persists_priorstate_undo(gov_home, monkeypatch):
    ...
    # The caller gets a digest; the config body stays out of the transcript.
    assert "hostname old-core" not in str(result["backup"])
