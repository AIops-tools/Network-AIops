...
|---|---|
| "Work read-only, never change any config" | Set `NETWORK_READ_ONLY=1`. The four write tools (`config_merge`, `config_replace`, `config_rollback`, `undo_apply`) are then **not registered at all** — 32 tools become 28, they never appear in the tool list, so the model cannot call one even if it tries. The `@governed_tool` harness independently refuses non-read calls, so the CLI is covered too. |
