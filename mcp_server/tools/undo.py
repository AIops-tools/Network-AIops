...
the inverse itself is destructive — tagged with its own risk tier in the audit
row). ``undo_apply`` is itself governed and audited like any other call.
...
def undo_apply(undo_id: str, dry_run: bool = False, target: Optional[str] = None) -> dict:
    ...
    recorded in that tool's audit row. Pass dry_run=True to preview the inverse
